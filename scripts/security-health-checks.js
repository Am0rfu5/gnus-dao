...
class SecurityHealthChecks {
  ...
  async runAllChecks(options = {}) {
    ...
    for (const [checkName, config] of Object.entries(this.config.schedules)) {
      if (config.enabled || force) {
        if (this.shouldRunCheck(checkName, config) || force) {
          this.log(`🔍 Running check: ${checkName}`);
          try {
            const checkResult = await this.runCheck(checkName, { verbose });
            ...
            this.log(
              `Error running check ${checkName}: ${error.message}`,
              "error",
            );
   ...
   */
  shouldRunCheck(checkName, config) {
    try {
   ...
   */
  async checkSecurityScan(options = {}) {
   ...
   */
  async checkContractAudit(options = {}) {
   ...
   */
  async checkAccessReview(options = {}) {
   ...
   */
  async checkDiamondIntegrity(options = {}) {
   ...
   */
  async checkNetworkSecurity(options = {}) {
   ...
   */
  updateLastRunTimestamp(checkName) {
   ...
   */
  getAssessmentMessage(status, score) {
    if (status === "healthy") {
   ...
   */
  async handleNotifications(results) {
   ...
   * Send health alert
   */
  async sendHealthAlert(results, type) {
...
async function main() {
  ...
  switch (command) {
    ...
    case "check":
      ...
      const result = await healthChecks.runCheck(checkName, { verbose: true });
    ...
    default:
      console.log("Usage:");
      console.log(
        "  node security-health-checks.js run [--force] [--verbose] [--no-report]",
      );
      console.log("  node security-health-checks.js check <check-name>");
      console.log("  node security-health-checks.js status");
      process.exit(1);
  }
}

// Export for use as module
module.exports = SecurityHealthChecks;
