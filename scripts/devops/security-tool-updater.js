...
class SecurityToolUpdater {
  ...
  async checkForUpdates() {
    ...
    for (const [toolName, tool] of Object.entries(this.tools)) {
      try {
        ...
        console.error(`❌ Error checking ${toolName}:`, error.message);
        this.updateLog.push({
          tool: toolName,
          error: error.message,
          status: "error",
  ...
  getCurrentVersion(tool) {
    try {
      ...
      console.warn(
        `Could not get current version for ${tool.name}:`,
        error.message,
      );
      return tool.current;
    }
  ...
  async getLatestVersion(toolName, type) {
    switch (type) {
      case "pip":
  ...
  async getGoLatestVersion(packageName) {
    ...
    for (const update of this.updateLog) {
      if (update.status === "pending") {
        ...
        try {
          ...
          console.error(`❌ Failed to update ${update.tool}:`, error.message);
        }
      }
    }
  ...
  async run() {
    try {
      ...
      console.error("💥 Critical error during security tool updates:", error);
      process.exit(1);
    }
  }
}
...
module.exports = SecurityToolUpdater;
