...
class CostAnalyzer {
  ...
  loadCostData() {
    ...
    try {
      ...
      console.error("❌ Error loading cost data:", error.message);
    }
  }
  ...
  calculateEstimatedCost(runtimeMinutes, runnerType = "ubuntu-latest") {
  ...
  analyzeCosts(timeframe = "all") {
    ...
    // Group by workflow
    const byWorkflow = filteredData.reduce((acc, item) => {
      acc[item.workflow] = acc[item.workflow] || [];
      acc[item.workflow].push(item);
      return acc;
    }, {});

    const workflowCosts = Object.entries(byWorkflow)
      .map(([workflow, runs]) => ({
        workflow,
  ...
  generateRecommendations(workflowCosts, allRuns) {
    ...
    expensiveWorkflows.forEach((workflow) => {
      recommendations.push({
        ...
        savings: workflow.totalCost * 0.3, // Estimate 30% savings
        suggestions: [
    ...
    longRunningWorkflows.forEach((workflow) => {
      recommendations.push({
        ...
        savings: (workflow.avgRuntime - 10) * workflow.runCount * 0.008, // Estimate savings with faster runtime
        suggestions: [
    ...
    if (failedRuns.length > allRuns.length * 0.1) {
      ...
      recommendations.push({
        ...
        savings: failedRuns.length * 0.1, // Estimate savings from reduced retries
        suggestions: [
    ...
    if (analysis.budgetStatus === "exceeded") {
      ...
      recommendations.push({
        ...
        message: `Approaching budget limit: $${analysis.budgetUsed} used of $${this.budgetLimit} limit`,
        suggestions: [
          "Review and optimize expensive workflows",
          "Consider upgrading to GitHub Team/Pro for better rates",
  ...
  generateReport(timeframe = "month") {
    console.log(
