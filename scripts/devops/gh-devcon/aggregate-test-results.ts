...
class TestResultsAggregator {
	...
	async aggregate(inputDir: string, outputFile: string): Promise<AggregatedResults> {
		console.log(`🔄 Aggregating test results from ${inputDir}...`);

		// Find all test result files
		const resultFiles: string[] = glob.sync('**/test-results-*.json', {
			cwd: inputDir,
		});
	...
	private async processResultFile(filePath: string): Promise<void> {
	...
	private determineCategory(filename: string): string {
		...
		if (filename.includes('deployment') || filename.includes('other')) return 'deployment';
		if (filename.includes('multichain')) return 'multichain';
		return 'other';
	}

	private determineNetwork(filename: string): string | null {
		const networks = ['ethereum', 'polygon', 'arbitrum', 'optimism'];
		return networks.find((network) => filename.includes(network)) || null;
	}

	private determineShard(filename: string): number | null {
		const shardMatch = filename.match(/shard-(\d+)/);
		return shardMatch ? parseInt(shardMatch[1]) : null;
	}

	private async aggregateCoverage(inputDir: string): Promise<void> {
		// Find all coverage files
		const coverageFiles: string[] = glob.sync('**/lcov.info', {
			cwd: inputDir,
			absolute: true,
		});
	...
	private calculateMetrics(): void {
		...
		this.results.success_rate =
			totalExecuted > 0 ? Math.round((this.results.passed / totalExecuted) * 100) : 0;

		// Calculate average time per test
		this.results.avg_time_per_test =
			this.results.total_tests > 0
				? Math.round((this.results.total_time / this.results.total_tests) * 100) / 100
				: 0;
		...
		Object.keys(this.results.categories).forEach((category) => {
			...
			cat.success_rate = catExecuted > 0 ? Math.round((cat.passed / catExecuted) * 100) : 0;
		});
		...
		Object.keys(this.results.networks).forEach((network) => {
			...
			net.success_rate = netExecuted > 0 ? Math.round((net.passed / netExecuted) * 100) : 0;
		});
	}

	generateReport(outputDir: string): TestReport {
		const report: TestReport = {
			summary: {
				...
				success_rate: this.results.success_rate || 0,
				coverage: this.results.coverage,
				total_time: this.results.total_time,
				avg_time_per_test: this.results.avg_time_per_test || 0,
			},
	...
	private findSlowestShard(): ShardInfo | null {
		if (this.results.shards.length === 0) return null;
		return this.results.shards.reduce((slowest, shard) =>
			shard.time > slowest.time ? shard : slowest,
		);
	}

	private findFastestShard(): ShardInfo | null {
		if (this.results.shards.length === 0) return null;
		return this.results.shards.reduce((fastest, shard) =>
			shard.time < fastest.time ? shard : fastest,
		);
	}
	...
	private generateRecommendations(): string[] {
		...
		if ((this.results.success_rate || 0) < 90) {
